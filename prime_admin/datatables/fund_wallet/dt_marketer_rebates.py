...
def fetch_marketer_rebates_dt():
    ...
    if branch_id == 'all':
        if current_user.role.name == "Admin":
            match = {'category': 'rebates'}
        elif current_user.role.name == "Partner":
            match = {'category': 'rebates', 'branch': {"$in": current_user.branches}}
        elif current_user.role.name == "Secretary":
            match = {'category': 'rebates' ,'branch': current_user.branch.id}
    else:
        if current_user.role.name == "Secretary":
            match = {'category': 'rebates', 'branch': current_user.branch.id}
        elif current_user.role.name == "Admin":
            match = {'category': 'rebates', 'branch': ObjectId(branch_id)}
        elif current_user.role.name == "Partner":
            match = {'category': 'rebates', 'branch': ObjectId(branch_id)}
